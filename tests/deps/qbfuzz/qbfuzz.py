...
def _init(comments):
    ...
    if _options.min_lits == None:
        ...
        elif _options.min_lits < math.ceil(
            _options.num_vars / _options.num_clauses
        ):
    ...
    if _options.max_lits == None:
        _options.max_lits = random.randint(
            _options.min_lits,
            max(_options.min_lits, min(_options.num_vars, MAX_LITS))
        )
...
def qbfuzz_main():
    ...
    if _options.free_ratio != None:
        num_free_vars = max(
            1, math.floor(_options.num_vars * _options.free_ratio)
        )
