...
class QBFDD:
    ...
    def __init__(
        ...
        if mode not in [
            self.DDMIN, self.SDDMIN, self.IDDMIN, self.ISDDMIN, self.OBO,
            self.QOBO
        ]:
    ...
    def dd(self):
        ...
        self._write_file(
            self.tmpfile, self.num_vars, self.quantsets, self.clauses
        )

        if self.verbose > 1:
            print(
                "\t\t{0}{1:7.2f}ms{2}".format(
        ...
        while 1:
                ...
                if self.gran != self.CONLY:
                    ...
                    for i in range(0, len(self.clauses)):
                        ...
                        if self.mode == self.DDMIN:
                            ...
                            (successful,
                             self.clauses[i]) = self._obo(clause, i)
                        elif self.mode == self.QOBO:
                            (successful, self.clauses[i]) = \
                    ...
                    if self.verbose:
                        ...
                        print(
                            "-{0} variable(s), -{1} literal(s) after "
            ...
            if not self.shift:
                ...
                if self.verbose:
                    ...
                    if successful:
                        print(
                            ": {0}successful{1}\t\t\t{2}{3:7.2f}s{4}".format(
        ...
        if self.compliant:
            if self.verbose > 1:
                start_upd = time.time()
                print("\nupdating in order to be QDIMACS compliant...", end='')
                sys.stdout.flush()
            ...
            if self.verbose > 1:
                print(
                    "\t\t\t{0}{1:7.2f}ms{2}".format(
                        self.gray, (time.time() - start_upd) * 1000, self.std
                    )
                )
        ...
        comments.append(
            "total: clauses  : -{0} ({1:.2%})".format(
                num_clauses_orig - len(self.clauses),
                1 - len(self.clauses) / num_clauses_orig
            )
        )
        comments.append(
            "       variables: -{0} ({1:.2%})".format(
                num_vars_orig - self.num_vars,
                1 - self.num_vars / num_vars_orig
            )
        )
        comments.append(
            "       literals : -{0} ({1:.2%})".format(
        ...
        self._write_file(
            self.outfile, self.num_vars, self.quantsets, self.clauses, comments
        )
                ...
                for i in range(len(subsets)):
                    ...
                    if self.compliant:
                        ...
                        if self.verbose > 1:
                            print(
                                "\t\t\t{0}{1:7.2f}ms{2}".format(
                                    self.gray,
                                    (time.time() - start_upd) * 1000, self.std
                                )
                            )
                    else:
                        count_upd = count
                    ...
                    if (len(count_upd) - 1) == 0:
                        ...
                        if self.verbose > 1:
                            start_write = time.time()
                            print(
                                "writing new configuration to output file...",
                                end=''
                            )
                            sys.stdout.flush()
                        ...
                        self._write_file(
                            self.tmpfile,
                            len(count_upd) - 1, quants_upd, clause_s_upd
                        )
                        ...
                        if self.verbose > 1:
                            print(
                                "test run: {0:4d}\t".format(self.test_runs),
                                end=''
                            )
    ...
    def _test_initial(self):
        ...
        try:
            file_idx = command.index("%")
            command[file_idx] = self.tmpfile
        except ValueError as v_err:
            raise ValueError(
                f"Given command '{self.cmd!s}' must include a % to represent"
                f" the file input"
            ) from v_err
        start = time.time()
    ...
    def _reduce(self, subsets, index, to_complement=True):
        ...
        assert (
            False not in
            (subset_item in self.clauses for subset_item in subsets[index])
        )
    ...
    def _minimize(self, clause, subsets, index, to_complement=True):
        ...
        assert (
            False
            not in (subset_item in clause for subset_item in subsets[index])
        )
    ...
    def _swap(self, index):
        ...
        if index > 0 and index < len(self.quantsets) - 1:
            ...
            if index < len(self.quantsets) - 1:
                assert (
                    self.quantsets[index][0] != self.quantsets[index + 1][0]
                )
            ...
            for i in range(len(self.quantsets)):
                ...
                for j in range(len(vars)):
                    ...
                    while 1:
                        ...
                        self._write_file(
                            self.tmpfile,
                            len(self.ref_count) - 1, quants, self.clauses
                        )
        ...
        try:
            with open(filename, 'r') as infile:
                ...
                if l[0] != 'p' or l[1] != "cnf" or len(l) != 4:
                    if l[0] == 'a' or l[0] == 'e':
                        raise QBFParseError(
                            "missing header", filename, num_lines
                        )
                ...
                try:
                    ...
                    if num_vars <= 0:
                        raise QBFParseError(
                            "invalid number of variables", filename, num_lines
                        )
                    ...
                    if num_clauses <= 0:
                        raise QBFParseError(
                            "invalid number of clauses", filename, num_lines
                        )
                except ValueError as err:
                ...
                for line in infile:
                    ...
                    if l[-1] != '0':
                        raise QBFParseError(
                            "missing '0' after quantset", filename, num_lines
                        )
                    elif len(l) == 2:
                        raise QBFParseError(
                            "empty quantset", filename, num_lines
                        )
                    ...
                    raise QBFParseError(
                        "missing clause definitions", filename, num_lines
                    )
                # clauses
                if l != self.BLANK_LINE: # first clause (already been read)
                    if l[-1] != '0':
                        raise QBFParseError(
                            "missing '0' after clause", filename, num_lines
                        )
                    try:
                        clause = []
                        for l_item in l[:-1]:
                            lit = self._parse_literal(
                                l_item, -num_vars, num_vars
                            )
                            clause.append(lit)
                            ref_count[abs(lit)] += 1
                ...
                if len(clauses) > num_clauses:
                    raise QBFParseError(
                        "too many clauses given", filename, num_lines
                    )
                elif len(clauses) < num_clauses:
                    raise QBFParseError(
                        "not enough clauses given", filename, num_lines
                    )
...
class QBFOptionParserFormatter(IndentedHelpFormatter):
    ...
    def format_description(self, description):
        if description:
            ...
            res = [
                textwrap.wrap(desc_item, self.width - self.current_indent)
...
class QBFOptionParser(OptionParser):
    ...
    def get_args_help(self):
        ...
        return (
            "\nArguments:\n" + "  INFILE" + gap
            + "input file in QDIMACS format\n" + "  CMD   " + gap
            + "call to QBF solver, options included\n"
        )
...
if __name__ == "__main__":
    ...
    try:
        qbf_dd = QBFDD(
            args[0], args[1], options.outfile, options.tmpfile, options.failed,
            options.passed, options.mode, options.gran, options.verbose,
            options.compliant, options.shift, options.skip, options.timeout,
