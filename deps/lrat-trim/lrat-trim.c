...
static struct Compression const
    compression_formats[NUM_COMPRESSION_FORMATS] = {
        bz2, gz, lzma, _7z, xz,
};
...
// Marcel: Many of the functions dealing with compression are taken from
// Armin
//         Biere's KISSAT. I modified them for readability and my own
//         purposes.
...
static struct Compression const *
file_probably_compressed (char const *const path) {
  assert (path != NULL);
...
static FILE *open_pipe (char const *const fmt, char const *const path,
  ...
  if (actual_len > max_len) {
    free (cmd);
    die ("Exceeded maximum buffer length while allocating "
         "compression command");
  }
...
void open_to_read_compressed (struct file *const file) {
  struct Compression const *type = file_probably_compressed (file->path);
  // Not compressed?
  if (type == NULL)
    file->pipe = 0, file->file = fopen (file->path, "r");
  else
    file->pipe = 1, file->file = open_pipe (type->cmd, file->path, "r");
  /* if (file->file == NULL) */
  /*   die ("Unable to open (potentially compressed) file '%s'",
   * file->path); */
}
...
static void parse_cnf () {
  ...
  PUSH (clauses.origins, 0);
  for (ch = read_first_char (); ch != 'p'; ch = read_ascii ()) {
    if (ch == 'c') {
      // Marcel: Try to read origin comment... or regular comment.
      while ((ch = read_ascii ()) == ' ')
        ;
      if (ch == EOF)
        prr ("end-of-file in header comment");
      else if (ch == 'o') {
        // Marcel: Make sure we never see more than one origin mapping
        //         comment
        if (found_origin)
          prr ("found multiple origin mapping comments produced in "
               "FERAT pipeline");
        // Marcel: Try to parse the rest of this comment as origin mapping
        found_origin = true;
        msg ("found origin mapping comment produced in FERAT "
             "pipeline");
        while (true) {
          while ((ch = read_ascii ()) == ' ')
            ;
          if (!ISDIGIT (ch))
            prr ("expected digit in FERAT mapping comment, not '%c'", ch);
          int origin = 0;
          while (ISDIGIT (ch)) {
            if (INT_MAX / 10 < origin)
            ORIGIN_INDEX_EXCEEDS_INT_MAX:
              prr ("index of FERAT origin clause '%s' exceeds 'INT_MAX'",
                   exceeds_int_max (origin, ch));
            origin *= 10;
            int digit = ch - '0';
            if (INT_MAX - digit < origin) {
              origin /= 10;
              goto ORIGIN_INDEX_EXCEEDS_INT_MAX;
            }
            origin += digit;
            ch = read_ascii ();
          }
          if (ch == EOF)
            prr ("end-of-file in FERAT origin mapping comment in header");
          else if (ch != ' ' && ch != '\t' && ch != '\n')
            prr ("expected white space after origin index '%d'", origin);
          // Marcel: Break when we see '0'
          if (!origin)
            break;
          PUSH (clauses.origins, origin);
        }
      }
    } else
      prr ("expected 'c' or 'p' as first character, not '%c'", ch);
    // Marcel: Consume rest of comment, if we didn't find an origin
    //         mapping, or the '\n' of an origin mapping comment
    while (ch != '\n') {
      if (ch == EOF)
  ...
  // Marcel: Since we have to parse the origin mapping comment before we see
  //         the 'p cnf' header, we need to check the number of indices
  //         retroactively
  int num_clause_origins = SIZE (clauses.origins) - 1;
  if (num_clause_origins == 0)
    ;
  else if (num_clause_origins > header_clauses)
    prr ("too many FERAT clause origins, got %d", num_clause_origins);
  else if (num_clause_origins < header_clauses)
    prr ("not every clause has a FERAT origin mapping, got %d",
         num_clause_origins);
  if (strict)
...
static void write_cnf () {
  ...
  int id;
  // Marcel: Write the origin mapping comment, omitting those clauses that
  //         were not used, just like the code below.
  //
  //         NOTE: For more information on this, see the code for
  //               'drat-trim', as it was modified for the FERAT paper by
  //               Simader and Seidl.
  if (!EMPTY (clauses.origins)) {
    vrb ("writing FERAT origin mapping comment to CNF");
    write_str ("c o ");
    for (id = 1; id <= last_clause_added_in_cnf && id <= empty_clause; ++id)
      if (ACCESS (clauses.used, id)) {
        assert (is_original_clause (id));
        write_int (ACCESS (clauses.origins, id));
        write_space ();
      }
    write_str ("0\n");
  }
...
static struct file *read_file (struct file *file) {
  ...
  if (!strcmp (file->path, "/dev/null")) {
    ...
    open_to_read_compressed (file);
    if (file->file == NULL)
      die ("Unable to open (potentially compressed) file '%s'", file->path);
  }
  file->saved = EOF;
  return file;
}
