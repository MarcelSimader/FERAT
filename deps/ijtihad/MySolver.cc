...
MySolver::MySolver(Prefix& pref, const CNF& cnf, const SolverOptions* options)
    ...
    if (options_.logging_phi) {
        try {
            ...
            phi_log_ << "c This file was generated by Ijtihad." << endl;
    ...
    if (options_.logging_ksi) {
        try {
            ...
            ksi_log_ << "c This file was generated by Ijtihad." << endl;
        } catch (std::exception& e) {
            cout << "c " << e.what();
    ...
    sat_solver_trivial_ = ipasir_init();

    debugn("Solver PHI is : " << sat_solver_phi_ << endl
                              << "Solver KSI is: " << sat_solver_ksi_);
    ...
    if (formula_->getNumQuants() % 2 != 1 ||
        formula_->getQuant(0).first != EXISTENTIAL) {
        debugn("Error: prefix size is "
               << formula_->getNumQuants() << ", first quantifier is "
               << ((formula_->getQuant(0).first == EXISTENTIAL) ? "EXISTENTIAL"
                                                                : "UNIVERSAL"));
        return;
    }
...
MySolver::~MySolver() {
    if (options_.logging_phi) phi_log_.close();
    if (options_.logging_ksi) ksi_log_.close();

    delete formula_;
    ipasir_release(sat_solver_phi_);
    ipasir_release(sat_solver_ksi_);
    ipasir_release(sat_solver_trivial_);
    for (vector<Var>* b : layer_caluese_bases_) delete b;
}

int MySolver::initPHI() {
    if (layer_size_ksi_ == 0) {
        for (unsigned clause_num = 0; clause_num < formula_->getNumClauses();
             clause_num++) {
            // Marcel: This undoes the renaming of literals in the trivially
            //         correct formula, so we don't have "gaps" between the
            //         variables.
            vector<Lit> renamed_clause;
            for (Lit const& l : formula_->getClause(clause_num))
                renamed_clause.emplace_back(
                    mkLit(renaming_scheme_[var(l)], sign(l)));
            addClause(sat_solver_trivial_, renamed_clause);
            origins_phi_.push_back(clause_num + 1);
        }
        return ipasir_solve(sat_solver_trivial_);
    }
    ...
    if (options_.warmup) {
        ...
        unsigned q_index = 0;
        for (unsigned quant_index = 0; quant_index < formula_->getNumQuants();
             quant_index++) {
            const Quantification& q = formula_->getQuant(quant_index);
            if (q.first == UNIVERSAL) {
                addt.assign(layer_size_ksi_, false);
                for (unsigned v_index = 0; v_index < q.second.size();
                     v_index++) {
                    addt[q_index] = true;
                    q_index += 1;
                }
...
void MySolver::generateDimacsPHI() {
    ...
    for (auto& p : vars_cache_phi_) {
        ...
        for (const Var& v : p.second) phi_log_ << v << " ";
...

        {
            ...
            while (j < p.first.size()) {
                ...
                if (q.first == UNIVERSAL) j += q.second.size();
                i++;
    ...
    phi_log_ << "c o ";
    for (unsigned origin : origins_phi_) phi_log_ << origin << " ";
    phi_log_ << "0" << std::endl;

    phi_log_ << "p cnf " << num_vars_phi_ << " " << num_clauses_phi_
             << std::endl;
...
void MySolver::printStats() {
    cout << "c sat_solver_phi calld " << sat_calls_ << " times, running for "
         << time_phi_ << " seconds." << endl;
    cout << "c number of layers in PHI is " << layers_phi_.size() << endl;
    cout << "c sat_solver_ksi was running for " << time_ksi_ << " seconds."
         << endl;
...
void MySolver::computeExtension(FormulaType type) {
    ...
    unsigned long per_call =
        (type == PHI) ? options_.cex_per_call : options_.wit_per_call;
    vector<vector<Var>>& layers = (type == PHI) ? layers_ksi_ : layers_phi_;
    unsigned long layer_size =
        (type == PHI) ? layer_size_ksi_ : layer_size_phi_;
    void* sat_solver = (type == PHI) ? sat_solver_ksi_ : sat_solver_phi_;
    vector<LitTuple>& to_add_container =
        (type == PHI) ? cex_to_add_ : wit_to_add_;
    ...
    // prepare extension
    for (long unsigned int layer_number = 0;
         added_layers < per_call && layer_number != layers.size();
         layer_number += 1) {
...
void MySolver::extendKSI() {
    ...
    bool trim_branch =
        (options_.trimming_ksi == SolverOptions::TrimmingMode::BRANCH_TRIMMING);
    if (trim_branch && independent_phi_.size() != 0 &&
        layers_phi_.size() != 0) {
        vector<bool> signs;
    ...
    if (trim_branch) old_wpc = trimKSI();

    computeExtension(KSI);

    // restore wpc
    if (trim_branch) options_.cex_per_call = old_wpc;
    ...
    for (auto& wit_it : wit_to_add_) {
        ...
        for (unsigned quant_index = 0; quant_index < formula_->getNumQuants();
            ...
            const Quantification& quant_inc =
                formula_->getCumulativeQuant(quant_index);
            ...
            if (quant.first == EXISTENTIAL) {
                ...
                if (cache_possible) {
                    ...
                    if (cache_iter != vars_cache_ksi_.end()) {
                        ...
                        vector<Var>::iterator subst_iter =
                            cache_iter->second.begin();
                        for (const Var& v :
                             formula_->getQuant(quant_index + 1).second) {
        ...
        for (unsigned clause_num = 0; clause_num < formula_->getNumClauses();
            ...
            for (const Lit& literal : formula_->getClausePHI(clause_num)) {
                if (sign(literal) ==
                    sign(wit_it[layerIndexPHI(var(literal))])) {
                    satisfied = true;
                    break;
                }
            }
            ...
            if (options_.tseitin_optimisation &&
                formula_->getClauseKSI(clause_num).size() == 1) {
                Lit literal = formula_->getClauseKSI(clause_num)[0];
                global_nand.push_back(mkLit(
                    layer_k[layerIndexKSI(var(literal))], !sign(literal)));
            } else {
                for (const Lit& literal : formula_->getClauseKSI(clause_num)) {
                    layer_cl_var->push_back(
                        layer_k[layerIndexKSI(var(literal))]);
                ...
                for (unsigned lit_index = 0; lit_index < layer_clause.size();
                     lit_index++) {
...
void MySolver::extendPHI() {
    ...
    bool trim_periodic = options_.trimming_phi ==
                             SolverOptions::TrimmingMode::PERIODIC_TRIMMING &&
                         (sat_calls_ % options_.trimming_interval == 1);
    bool trim_dynamic =
        (options_.trimming_phi ==
             SolverOptions::TrimmingMode::DYNAMIC_TRIMMING &&
         ((read_mem_usage() > options_.memory_limit) ||
          (sat_calls_ % options_.trimming_interval == 1) ||
          (sat_calls_ > 0 && layers_phi_.size() > 1.2 * layers_ksi_.size())));

    bool trim_branch =
        (options_.trimming_phi == SolverOptions::TrimmingMode::BRANCH_TRIMMING);
    if (trim_branch && independent_phi_.size() == 0 &&
        formula_->getNumQuants() != 3 && layers_ksi_.size() != 0) {
        vector<bool> signs;
        // ksi indices for top will have to start from 1
    ...
    if (trim_branch) cout << "Assignment to KSI changed, trimming PHI" << endl;
    ...
    for (auto& cex_it : cex_to_add_) {
        ...
        for (unsigned int i = 0; i < independent_phi_.size(); i++) {
            ...
            if (options_.bumping) ipasir_bump(sat_solver_phi_, i + 1);
        }
        ...
        for (unsigned quant_index = 1; quant_index < formula_->getNumQuants();
             quant_index++) {
            const Quantification& quant = formula_->getQuant(quant_index);
            const Quantification& quant_inc =
                formula_->getCumulativeQuant(quant_index);
            ...
            if (quant.first == UNIVERSAL) {
                ...
                if (cache_possible) {
                    ...
                    if (cache_iter != vars_cache_phi_.end()) {
                        ...
                        debugn("We have a variable cache hit: " << quant_index +
                                                                       1);
                        debug({
        ...
        for (unsigned clause_num = 0; clause_num < formula_->getNumClauses();
            ...
            for (const Lit& literal : formula_->getClauseKSI(clause_num)) {
                if (sign(literal) ==
                    sign(cex_it[layerIndexKSI(var(literal))])) {
                    satisfied = true;
                    break;
                }
            }
            ...
            if (formula_->getClauseKSI(clause_num).empty() &&
                !formula_->getClausePHI(clause_num).empty() &&
                layers_phi_.size() > 1 &&
                formula_->getDepthPHI(clause_num) <= deepest_match) {
            ...
            for (const Lit& literal : formula_->getClausePHI(clause_num)) {
                cl.push_back(
                    mkLit(layer_p[layerIndexPHI(var(literal))], sign(literal)));
            }
...
void MySolver::addClause(void* solver, const vector<Lit>& clause) {
    if (options_.logging_phi &&
        (solver == sat_solver_phi_ || solver == sat_solver_trivial_)) {
...
void MySolver::printVersion() { cout << "c MySolver" << VERSION << std::endl; }
...
void MySolver::renameVariables(Prefix& pref, const CNF& cnf) {
    ...
    for (const Quantification& q : pref) {
        unsigned* x = (q.first == EXISTENTIAL) ? &(renaming_size[0])
                                               : &(renaming_size[1]);
        *x += q.second.size();
        for (const Var& v : q.second) {
            if (v > max_id_) max_id_ = v;
        }
    }

    renaming_scheme_.resize(2 * std::max(renaming_size[0], renaming_size[1]) +
                            1);

    formula_->setVariableDepthSize(
        2 * std::max(renaming_size[0], renaming_size[1]) + 1);
    ...
    for (const Quantification& q : pref) {
        ...
        Quantification& quant_inc =
            (q.first == EXISTENTIAL) ? quant_inc_e : quant_inc_a;
